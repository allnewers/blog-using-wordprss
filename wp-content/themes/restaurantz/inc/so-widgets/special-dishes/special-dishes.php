...
class Restaurantz_Special_Dishes_Widget extends SiteOrigin_Widget {
		...
		parent::__construct(
			...
			array(
				...
				'dishes_post_id' => array(
					...
					'description' => sprintf( __( 'Choose %sPost Type%s as %sMenu Items%s while building query to select dishes.', 'restaurantz' ), '<strong>', '</strong>', '<strong>', '</strong>' ),
				),
	...
	function get_valid_menu_ids( $instance ) {
		...
		if ( isset( $raw_select_ids['post__in'] ) && ! empty( $raw_select_ids['post__in'] ) ) {
			$dids = $raw_select_ids['post__in'];
			if ( ! empty( $dids ) ) {
				for ( $i = 0; $i < count( $dids ); $i++ ) {
					if ( 'nova_menu_item' === get_post_type( $dids[ $i ] ) ) {
						$valid_select_ids[] = $dids[ $i ];
					}
				}
			}
		}
